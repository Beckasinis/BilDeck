export default function RetentionCards() {
  const methods = [   
    ...
    },
    {
      ...
      label: 'Active recall',
      description:
        'Upp till 2–3× bättre långtidsretention. Hjärnan tvingas hämta svaret aktivt, vilket stärker minnessporet varje gång.',
      accent: true,
    ...
    <div className="retention-cards">
      {methods.map(m => (
        <div key={m.label} className={`retention-card${m.accent ? ' retention-card--accent' : ''}`}>
          ...
          {m.accentLabel && <span className="retention-badge retention-badge--muted">{m.accentLabel}</span>}
